...
- 電源プラン / タイマー分解能設定 / 同時実行していた負荷: **記録なし**
  （このため絶対値は環境間の比較には使えない。同一表内の相対比較にのみ用いること）

> **本ドキュメントの数値は 2026-03-11 時点・v0.2.0 の実測値であり、v0.2.1 では再測定していない。**
> v0.2.1 の変更（バージョン照会 API の追加、`USLEEPWIN_STATIC`、呼び出し規約の明示、
> `usleep_set_power_mode()` の定数修正、HR タイマー可用性判定の `probe_hrtimer_support()` 化）のうち、
> **HR タイマー可用性判定の変更は待機経路の選択に影響しうる**（従来は短い待機しかしないスレッドが
> タイマー経路に入れず純スピンしていた）。v0.2.1 での再測定は**予定**であり、本ページの数値は
> それまで v0.2.0 のものとして扱うこと。

> 注: Windows はハードリアルタイム OS ではない。以下はこの 1 台・この設定での一度きりのスナップショットであり、
> 精度を保証するものではない。
...
> **本測定はいずれも電力モード未適用の状態**である。定数は v0.2.1 で 3 に修正された。
